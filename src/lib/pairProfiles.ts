...
export const PAIR_PROFILES: Record<string, PairProfile> = {
  ...
  // ---- Tier 1 (calm) ----
  EURUSD: fx("EURUSD", 1, 20, 1.6, ["London", "New York"]),
  USDCHF: fx("USDCHF", 1, 20, 1.6, ["London", "New York"], {
    disabled: true, disabledReason: "Pause new entries — audit before re-enabling (v3: worst pair, PF 0.12)",
  }),
  EURGBP: fx("EURGBP", 1, 18, 1.7, ["London"]),
  EURCHF: fx("EURCHF", 1, 18, 1.8, ["London"], { note: "CHF cluster — monitor closely" }),
  AUDCHF: fx("AUDCHF", 1, 20, 1.6, ["Sydney", "London"], { note: "Best CHF-cluster pair" }),
  CADCHF: fx("CADCHF", 1, 19, 1.7, ["New York", "London"], { note: "CHF cluster — monitor" }),
  AUDNZD: fx("AUDNZD", 1, 18, 1.8, ["Sydney"]),
  NZDCHF: fx("NZDCHF", 1, 19, 1.7, ["Sydney", "London"], { note: "CHF cluster — monitor" }),

  // ---- Tier 2 (medium) ----
  GBPUSD: fx("GBPUSD", 2, 23, 1.5, ["London", "New York"], { note: "Flipped to losing — watch" }),
  USDCAD: fx("USDCAD", 2, 23, 1.5, ["New York"], { note: "Flipped to flat — watch" }),
  AUDUSD: fx("AUDUSD", 2, 22, 1.5, ["Sydney", "London"]),
  NZDUSD: fx("NZDUSD", 2, 22, 1.5, ["Sydney", "London"]),
  EURAUD: fx("EURAUD", 2, 23, 1.5, ["London"]),
  EURCAD: fx("EURCAD", 2, 23, 1.5, ["London", "New York"]),
  EURNZD: fx("EURNZD", 2, 23, 1.5, ["London"]),
  GBPCHF: fx("GBPCHF", 2, 23, 1.5, ["London"], { note: "CHF cluster — monitor" }),
  AUDCAD: fx("AUDCAD", 2, 22, 1.5, ["Sydney", "New York"]),
  NZDCAD: fx("NZDCAD", 2, 22, 1.5, ["Sydney", "New York"]),

  // ---- Tier 3 (volatile) ----
  USDJPY: fx("USDJPY", 3, 25, 1.3, ["Tokyo", "New York"]),
  EURJPY: fx("EURJPY", 3, 25, 1.3, ["Tokyo", "London"]),
  GBPAUD: fx("GBPAUD", 3, 26, 1.2, ["London"]),
  GBPCAD: fx("GBPCAD", 3, 25, 1.3, ["London", "New York"], { note: "Flipped to badly losing — watch" }),
  GBPJPY: fx("GBPJPY", 3, 28, 1.1, ["Tokyo", "London"], {
    disabled: true, disabledReason: "Pause new entries — v3 stop-widening did not help; needs independent review",
  }),
  GBPNZD: fx("GBPNZD", 3, 26, 1.2, ["London"]),
  AUDJPY: fx("AUDJPY", 3, 25, 1.3, ["Tokyo", "Sydney"]),
  CADJPY: fx("CADJPY", 3, 25, 1.3, ["Tokyo", "New York"], { note: "Flipped to losing — watch" }),
  CHFJPY: fx("CHFJPY", 3, 24, 1.3, ["Tokyo", "London"]),
  NZDJPY: fx("NZDJPY", 3, 24, 1.3, ["Tokyo", "Sydney"]),
};
